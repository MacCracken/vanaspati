...
- **bridge** — `nitrogen_to_growth_stress(plant_n, critical_n)` cross-crate N stress; `soil_water_to_photosynthesis_stress(rwc)` and `soil_water_to_growth_stress(rwc)` water stress bridges; pravash bridges: `wind_to_boundary_conductance`, `humidity_to_vpd`
