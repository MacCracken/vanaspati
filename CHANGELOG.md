...
- **photosynthesis** — `PhotosynthesisPathway` enum (C3, C4, CAM), `pathway_params` returning (optimum_temp, quantum_yield, max_rate), `temperature_factor_c4` (σ²=150, optimum 32°C), `temperature_factor_cam` (σ²=250, optimum 28°C); Beer-Lambert canopy light competition: `canopy_light_at_depth`, `understory_light_fraction`, `light_interception`, `shaded_photosynthesis_rate`
