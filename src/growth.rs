...
#[non_exhaustive]
pub enum GrowthStage { Seed, Germination, Seedling, Vegetative, Flowering, Fruiting, Senescence, Dormant }
...
impl GrowthModel {
    ...
    pub fn height_at_day(&self, day: f32) -> f32 {
        if self.initial_height <= 0.0 { return 0.0; }
        let k = self.max_height;
        let h0 = self.initial_height;
        let r = self.growth_rate;
        k / (1.0 + ((k - h0) / h0) * (-r * day).exp())
    }

    /// Growth rate at current height (derivative of logistic).
    #[must_use]
    pub fn daily_growth(&self, current_height: f32) -> f32 {
        let k = self.max_height;
        if k <= 0.0 { return 0.0; }
        self.growth_rate * current_height * (1.0 - current_height / k)
    }

    /// Oak tree: slow growth, tall.
    #[must_use]
    pub fn oak() -> Self { Self { max_height: 25.0, growth_rate: 0.005, initial_height: 0.1 } }
    /// Bamboo: fast growth.
    #[must_use]
    pub fn bamboo() -> Self { Self { max_height: 20.0, growth_rate: 0.05, initial_height: 0.1 } }
    /// Grass: very fast, short.
    #[must_use]
    pub fn grass() -> Self { Self { max_height: 0.5, growth_rate: 0.1, initial_height: 0.01 } }
}
...
mod tests {
    ...
    fn logistic_approaches_max() {
        ...
        assert!((h - m.max_height).abs() < 0.01, "should approach max height, got {h}");
    }
